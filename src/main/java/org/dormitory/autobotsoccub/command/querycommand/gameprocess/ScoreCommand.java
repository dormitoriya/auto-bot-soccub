...
public class ScoreCommand implements Command {
    ...
    private static final String GOAL_SCORED = "%s scored!";
    ...
    public void execute(ChatData chatData) {
        if (GOAL.getText().equals(chatData.getCommandName())) {
            chatData.getGameEngineOperations().score( chatData.getUser().getId());

            chatData.setCommandResult(
                    CommandResult.builder()
                            .replyKeyboardMarkup(keyboardFactory.replyKeyboardOf(GOAL, AUTO_GOAL, SUBSTITUTION, REVERT, STOP))
                            .gameStateImage(updateImage(format(GOAL_SCORED,  chatData.getUser().getFirstName()), chatData))
                            .build());
